    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-5 space-y-5">
          ...
          {/* Live Buzzer Queue Card (Realtime Ranking) */}
          <div className="bg-white/95 backdrop-blur rounded-3xl border border-slate-200/90 p-5 sm:p-6 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <Bell className="w-4 h-4 text-rose-600 animate-bounce shrink-0" />
                <h3 className="text-xs font-black text-slate-800 uppercase tracking-wider font-mono">
                  Hàng Đợi Chuông Phản Biện ({buzzerQueue.length})
                </h3>
              </div>
              <div className="flex items-center gap-2">
                {buzzerQueue.length > 0 && onResetBuzzer && (
                  <button
                    onClick={() => {
                      onResetBuzzer();
                      soundManager.playDing();
                    }}
                    className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 border border-slate-300 text-slate-700 font-bold text-xs transition-colors cursor-pointer"
                    title="Xóa danh sách chuông"
                  >
                ...
                {onOpenTeamBuzzer && (
                  <button
                    ...
                    className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-rose-50 hover:bg-rose-100 border border-rose-200 text-rose-700 font-bold text-xs transition-colors cursor-pointer"
                  >
                    <Bell className="w-3 h-3 text-rose-600" />
                    <span>Nút Chuông 10 Đội</span>
                  </button>
                )}
              </div>
            </div>
            ...
            {buzzerQueue.length === 0 ? (
              <div className="p-4 rounded-2xl bg-emerald-50/70 border border-emerald-200/80 text-center space-y-2.5">
                <div className="flex items-center justify-center gap-2 text-emerald-800 text-xs font-bold">
                  <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                  <span>Chuông Mở Sẵn Sàng — 10 đội có thể bấm chuông bất kỳ lúc nào!</span>
                </div>
                <p className="text-[11px] text-slate-500">
                  Bấm thử nhanh trực tiếp trên màn hình sân khấu:
                </p>
                <div className="flex flex-wrap items-center justify-center gap-1.5 pt-1">
                  {teams
                    ...
                    .map((t) => (
                      <button
                        key={t.id}
                        onClick={() => onBuzz?.(t.id, t.name)}
                        className="px-2.5 py-1 rounded-xl text-xs font-bold bg-white hover:bg-rose-50 border border-slate-200 hover:border-rose-300 text-slate-700 hover:text-rose-700 transition-all shadow-2xs active:scale-95 flex items-center gap-1 cursor-pointer"
                        title={`Bấm chuông thử cho ${t.name}`}
                      >
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center justify-between p-3 rounded-2xl border transition-all ${
                        isFirst
                          ? 'bg-amber-50/90 border-amber-300 shadow-xs ring-1 ring-amber-300'
                          : isSecond
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            {isFirst && (
                              <span className="px-2 py-0.5 rounded-full bg-amber-200 border border-amber-300 text-amber-900 text-[10px] font-black uppercase tracking-wider animate-pulse">
                                Nhanh nhất ⚡
                              </span>
                            )}
                          </div>
                          <span className="text-[11px] font-mono text-slate-500">
                            {record.diffMs === 0 ? '0.00s' : `+${(record.diffMs / 1000).toFixed(2)}s`}
                          </span>
                        </div>
                      </div>

                      <button
                        onClick={() => setSelectedDebaterTeamId(record.teamId)}
                        className="px-3 py-1.5 rounded-xl bg-gradient-to-r from-rose-600 to-amber-600 hover:from-rose-500 hover:to-amber-500 text-white text-xs font-extrabold shadow-xs transition-all active:scale-95 flex items-center gap-1.5 cursor-pointer"
                      >
                        <Flame className="w-3.5 h-3.5" />
                        <span>Chọn Phản Biện</span>
                      </button>
                    </div>
                  );
                })}

                <div className="pt-2 border-t border-slate-100 flex flex-wrap items-center justify-center gap-1.5">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mr-1">
                    Bấm thêm:
                  </span>
                  {teams
                    .filter((t) => t.id !== currentTeam.id && !buzzerQueue.some((b) => b.teamId === t.id))
                    .map((t) => (
                      <button
                        key={t.id}
                        onClick={() => onBuzz?.(t.id, t.name)}
                        className="px-2 py-0.5 rounded-lg text-[10px] font-semibold bg-slate-50 hover:bg-rose-50 border border-slate-200 hover:border-rose-300 text-slate-600 hover:text-rose-700 transition-all cursor-pointer"
                        title={`Bấm chuông cho ${t.name}`}
                      >
                        + {t.name}
                      </button>
                    ))}
